...
let manageBorrowsBtn;
let borrowTableBody;
let manageBorrowModal, closeManageBorrowModalBtn, closeManageBorrowModalCancelBtn, manageBorrowTableBody, openAddBorrowBtn;
let addBorrowModal, closeAddBorrowModalBtn, borrowerStudentSelect, borrowDatetimeInput, daysUntilReturnInput, dueDateInput, borrowAmountInput, saveBorrowBtn, cancelAddBorrowBtn;
...
let historyWebsideBtn = document.getElementById('history-webside');
let closeHistoryModalBtn = document.getElementById('closeHistoryModalBtn');
let historyTableBody = document.getElementById('historyTableBody');
let historywebsideLimitInput = document.getElementById('historyLimitInput');
let autoDeleteSwitch = document.getElementById('autoDeleteSwitch');
let clearHistorywebsideBtn = document.getElementById('clearHistoryVisitBtn');
let refreshHistoryBtn = document.getElementById('refreshHistoryBtn');
...
async function recordWebsiteVisit() {
  try {
    // ตรวจสอบว่า db (Firestore) พร้อมใช้งานหรือยัง
    if (typeof db === 'undefined' || !db) {
      console.warn('Firestore (db) not initialized, skipping visit record.');
      return;
    }

    // ป้องกันการบันทึกซ้ำซ้อน ถ้ารีเฟรชหน้าเดิมภายใน 1 นาที
    const VISIT_DEDUPE_KEY = 'lastVisit';
    const MIN_INTERVAL_MS = 60 * 1000; // 1 นาที
    const now = Date.now();
    const currentPath = location.pathname + location.search;
    const last = JSON.parse(localStorage.getItem(VISIT_DEDUPE_KEY) || '{}');
    if (last.path === currentPath && (now - (last.ts || 0)) < MIN_INTERVAL_MS) {
      // console.log('Skipping duplicate visit record.');
      return; // ออกจากฟังก์ชันถ้าเข้าชมซ้ำเร็วเกินไป
    }

    const colRef = db.collection('websiteVisits');

    // ✅ พยายามดึง IP address
    ...
    try {
      ...
      // ตรวจจับ OS
      if (/Windows NT 10.0/i.test(ua)) osVersion = 'Windows 10';
      else if (/Windows NT 11.0/i.test(ua)) osVersion = 'Windows 11';
      else if (/Windows NT 6.3/i.test(ua)) osVersion = 'Windows 8.1';
      else if (/Windows NT 6.1/i.test(ua)) osVersion = 'Windows 7';
      else if (/Mac OS X (\d+[_.]\d+)/i.test(ua)) {
        osVersion = 'macOS ' + ua.match(/Mac OS X (\d+[_.]\d+)/i)[1].replace('_', '.');
      } else if (/Android ([0-9.]+)/i.test(ua)) {
        osVersion = 'Android ' + ua.match(/Android ([0-9.]+)/i)[1];
      } else if (/CPU iPhone OS ([0-9_]+)/i.test(ua)) {
        osVersion = 'iOS ' + ua.match(/CPU iPhone OS ([0-9_]+)/i)[1].replaceAll('_', '.');
      }

      // ตรวจจับชื่ออุปกรณ์/รุ่น
      if (/iPhone/i.test(ua)) deviceName = 'iPhone';
      else if (/iPad/i.test(ua)) deviceName = 'iPad';
      else if (/Samsung/i.test(ua)) deviceName = 'Samsung ' + (ua.match(/SM-[A-Z0-9]+/i)?.[0] || '');
      else if (/Huawei/i.test(ua)) deviceName = 'Huawei';
      else if (/Xiaomi/i.test(ua)) deviceName = 'Xiaomi';
      else if (/OPPO/i.test(ua)) deviceName = 'OPPO';
    ...
    // ดึงชื่อที่แสดงในแชท (ถ้ามี)
    const displayName = (typeof chatDisplayName !== 'undefined' && chatDisplayName)
      ? chatDisplayName
      : (localStorage.getItem('chatDisplayName') || 'guest');

    // ✅ ข้อมูลที่จะบันทึกลง Firestore
    const visitData = {
      ipAddress,
      device: deviceName,
      osVersion,
    ...
    // บันทึกข้อมูล
    await colRef.add(visitData);
    // เก็บเวลาเข้าชมล่าสุดไว้ใน localStorage เพื่อกันการบันทึกซ้ำ
    localStorage.setItem(VISIT_DEDUPE_KEY, JSON.stringify({ path: currentPath, ts: now }));

    console.log('✅ Website visit recorded with detailed device info:', visitData);

  } catch (err) {
    console.error('❌ Error recording website visit:', err);
  }
}

// Call the function on page load
window.onload = recordWebsiteVisit;

// ...existing code...
async function renderHistoryTable() {
    try {
        // ตรวจสอบว่า element 'historyTableBody' พร้อมใช้งาน
        if (!historyTableBody) historyTableBody = document.getElementById('historyTableBody');
        if (!historyTableBody) return;

        // แสดงสถานะ "กำลังโหลด..."
        historyTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">กำลังโหลด...</td></tr>';

        // ตรวจสอบว่า db (Firestore) พร้อมใช้งาน
        if (typeof db === 'undefined' || !db) {
            historyTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:red;">Firestore ยังไม่ได้เชื่อมต่อ</td></tr>';
            return;
        }

        let colRef = db.collection('websiteVisits');
        // ดึงข้อมูล 100 รายการล่าสุด โดยเรียงจากใหม่ไปเก่า
        const snapshot = await colRef.orderBy('timestamp', 'desc').limit(100).get();

        historyTableBody.innerHTML = ''; // ล้างสถานะ "กำลังโหลด..."
        ...
        if (snapshot.empty) {
            ...
            cell.colSpan = 5;
            cell.textContent = 'ยังไม่มีประวัติการเข้าชม';
        ...
        snapshot.forEach(doc => {
            ...
            // [REVISED] ดึงข้อมูลให้ตรงกับ 5 คอลัมน์ที่อัปเดต
            const ip = data.ipAddress || data.ip || data.clientIp || '-';
            const device = data.device || data.deviceName || '-'; // สำหรับ 'ชื่ออุปกรณ์ (รุ่น)'
            const osVersion = data.osVersion || '-'; // สำหรับ 'เวอร์ชั่น OS'
            const name = data.displayName || data.chatName || data.name || '-'; // สำหรับ 'รหัสผู้ใช้'

            let tsText = '-'; // สำหรับ 'วัน-เวลา'
            if (data.timestamp && typeof data.timestamp.toDate === 'function') {
                tsText = data.timestamp.toDate().toLocaleString('th-TH');
            } else if (data.visit_date) {
                try { tsText = new Date(data.visit_date).toLocaleString('th-TH'); } catch (e) { tsText = String(data.visit_date); }
            } else if (data.createdAt && typeof data.createdAt.toDate === 'function') {
                tsText = data.createdAt.toDate().toLocaleString('th-TH');
            }

            // [REVISED] สร้าง 5 เซลล์ตามลำดับของ HTML: วัน-เวลา, ชื่ออุปกรณ์, เวอร์ชั่น OS, IP, รหัสผู้ใช้
            const row = historyTableBody.insertRow();
            row.insertCell(0).textContent = tsText;
            row.insertCell(1).textContent = device;
            row.insertCell(2).textContent = osVersion;
            row.insertCell(3).textContent = ip;
            row.insertCell(4).textContent = name;
        });
    } catch (error) {
        ...
        if (historyTableBody) {
            ...
            const cell = row.insertCell(0);
            cell.colSpan = 5;
            cell.textContent = `เกิดข้อผิดพลาดในการโหลดประวัติ: ${error.message || error}`;
...
// ฟังก์ชันสำหรับโหลดการตั้งค่าจาก Firebase
async function loadHistorySettings() {
    try {
        if (typeof appSettingsDocRef === 'undefined') return;
        const docSnap = await appSettingsDocRef.get();
        const settings = docSnap && docSnap.exists ? docSnap.data() : {};

        const visitLimitEl =
            document.getElementById('historyModalLimitInput') ||
            document.getElementById('historyLimitInput');

        const autoDeleteVisitEl =
            document.getElementById('autoDeleteModalSwitch') ||
            document.getElementById('autoDeleteToggle');

        if (visitLimitEl) {
            visitLimitEl.value = typeof settings.visitLimit !== 'undefined'
                ? settings.visitLimit
                : (typeof settings.historyLimit !== 'undefined' ? settings.historyLimit : 50);
        }
        if (autoDeleteVisitEl) {
            autoDeleteVisitEl.checked = !!(settings.autoDeleteVisits || settings.autoDelete || settings.autoDeleteEnabled);
        }
    } catch (err) {
...
async function clearAllHistory() {
    if (!confirm('คุณแน่ใจหรือไม่ว่าต้องการลบประวัติการเข้าชมทั้งหมด?')) return;
    try {
        const col = db.collection('websiteVisits');
        const snapshot = await col.get();
        if (snapshot.empty) {
            alert('ไม่มีข้อมูลประวัติการเข้าชมให้ลบ');
            return;
        }
        let batch = db.batch();
        let counter = 0;
        ...
        console.log(`Cleared websiteVisits (${snapshot.size} docs).`);
        if (typeof renderHistoryTable === 'function') await renderHistoryTable();
        alert('ลบประวัติการเข้าชมเรียบร้อยแล้ว');
    } catch (error) {
        console.error('Error clearing all website visit history:', error);
        alert('เกิดข้อผิดพลาดในการลบประวัติ ดู console สำหรับรายละเอียด');
    }
}
// ...existing code...
...
async function renderBorrowTable() {
    try {
        if (!borrowTableBody) borrowTableBody = document.getElementById('borrowTableBody');
        if (!borrowTableBody) return;

        borrowTableBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">กำลังโหลด...</td></tr>';

        if (typeof db === 'undefined' || !db || typeof borrowsCollection === 'undefined') {
            borrowTableBody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:red;">Firestore ยังไม่ได้เชื่อมต่อ</td></tr>';
            return;
        }

        const snapshot = await borrowsCollection.orderBy('borrowedAt', 'desc').limit(100).get();
        borrowTableBody.innerHTML = '';

        if (snapshot.empty) {
            const r = borrowTableBody.insertRow();
            const c = r.insertCell(0);
            c.colSpan = 6;
            c.textContent = 'ยังไม่มีรายการยืม';
        ...
        snapshot.forEach(doc => {
            ...
            // วันที่ยืม
            let borrowedAt = '-';
            if (d.borrowedAt && typeof d.borrowedAt.toDate === 'function') borrowedAt = d.borrowedAt.toDate().toLocaleString('th-TH');
            else if (d.borrow_date) borrowedAt = String(d.borrow_date);

            // กำหนดคืน
            let due = '-';
            if (d.dueDate && typeof d.dueDate.toDate === 'function') due = d.dueDate.toDate().toLocaleDateString('th-TH');
            else if (d.dueDateString) due = String(d.dueDateString);

            // วันที่คืน (returnedAt)
            let returnedAt = '-';
            if (d.returnedAt && typeof d.returnedAt.toDate === 'function') returnedAt = d.returnedAt.toDate().toLocaleString('th-TH');

            // ผู้ยืม
            const borrower = d.borrowerName || d.borrowerId || '-';

            // จำนวนเงิน: รองรับทั้ง amountBorrowed / amount / amountBorrowed-string
            let amountVal = null;
            if (typeof d.amountBorrowed === 'number') amountVal = d.amountBorrowed;
            else if (typeof d.amount === 'number') amountVal = d.amount;
            else if (typeof d.amountBorrowed === 'string' && d.amountBorrowed.trim() !== '') amountVal = Number(d.amountBorrowed);
            else if (typeof d.amount === 'string' && d.amount.trim() !== '') amountVal = Number(d.amount);

            const amountText = (typeof amountVal === 'number' && !isNaN(amountVal))
                ? amountVal.toLocaleString('th-TH')
                : '-';

            // สถานะ (คืนแล้ว / ยังไม่คืน)
            const statusText = d.returned ? 'คืนแล้ว' : 'ยังไม่คืน';
            const statusClass = d.returned ? 'status-returned' : 'status-pending';

            // สร้าง 6 เซลล์: วันที่ยืม, กำหนดคืน, วันที่คืน, ผู้ยืม, จำนวน, สถานะ
            const tdBorrowed = document.createElement('td'); tdBorrowed.textContent = borrowedAt;
            const tdDue = document.createElement('td'); tdDue.textContent = due;
            const tdReturned = document.createElement('td'); tdReturned.textContent = returnedAt;
            const tdBorrower = document.createElement('td'); tdBorrower.textContent = borrower;
            const tdAmount = document.createElement('td'); tdAmount.style.textAlign = 'right'; tdAmount.textContent = amountText;
            const tdStatus = document.createElement('td'); tdStatus.textContent = statusText; tdStatus.className = statusClass;
        ...
        borrowTableBody.appendChild(frag);
    } catch (err) {
        console.error('renderBorrowTable error', err);
        if (borrowTableBody) {
            borrowTableBody.innerHTML = '';
            const r = borrowTableBody.insertRow();
            const c = r.insertCell(0);
            c.colSpan = 6;
            c.textContent = `เกิดข้อผิดพลาด: ${err.message || err}`;
            c.style.color = 'red';
            c.style.textAlign = 'center';
        }
    }
...
// Modal: render full list inside manage modal
// ...existing code...
async function renderManageBorrowTable() {
    try {
        if (!manageBorrowTableBody) manageBorrowTableBody = document.getElementById('manageBorrowTableBody');
        if (!manageBorrowTableBody) return;

        const snapshot = await borrowsCollection.orderBy('borrowedAt', 'desc').get();
        manageBorrowTableBody.innerHTML = '';

        if (snapshot.empty) {
            const r = manageBorrowTableBody.insertRow();
            const c = r.insertCell(0);
            c.colSpan = 7;
            c.textContent = 'ยังไม่มีรายการยืม';
            c.style.textAlign = 'center';
        ...
        snapshot.forEach(doc => {
            const d = doc.data() || {};
            const id = doc.id;
            const row = manageBorrowTableBody.insertRow();

            const borrowedAt = d.borrowedAt && d.borrowedAt.toDate ? d.borrowedAt.toDate().toLocaleString('th-TH') : (d.borrow_date || '-');
            const due = d.dueDate && d.dueDate.toDate ? d.dueDate.toDate().toLocaleDateString('th-TH') : (d.dueDateString || '-');
            const returnedAt = d.returnedAt && d.returnedAt.toDate ? d.returnedAt.toDate().toLocaleString('th-TH') : '-';

            row.insertCell(0).textContent = borrowedAt;
            row.insertCell(1).textContent = due;
            row.insertCell(2).textContent = returnedAt;
            row.insertCell(3).textContent = d.borrowerName || '-';

            // amountBorrowed cell
            const amountBorrowedCell = row.insertCell(4);
            amountBorrowedCell.style.textAlign = 'right';
            amountBorrowedCell.textContent = (typeof d.amountBorrowed !== 'undefined') ? d.amountBorrowed : (typeof d.amount !== 'undefined' ? d.amount : '-');


            const actionCell = row.insertCell(5);

            // No "หักเงิน" button — only allow marking as returned (change status) or show label
            if (!d.returned) {
                ...
                returnBtn.onclick = async () => {
                    if (!confirm('ยืนยันว่ารายการนี้คืนแล้วและจะถูกทำเครื่องหมายเป็นคืนแล้ว?')) return;
                    try {
                        await borrowsCollection.doc(id).update({
                            returned: true,
                            returnedAt: firebase.firestore.FieldValue.serverTimestamp()
                        });
                        await renderManageBorrowTable();
                        await renderBorrowTable();
                    } catch (e) {
                        console.error('mark as returned error', e);
                        alert('เกิดข้อผิดพลาดในการบันทึกสถานะคืนแล้ว');
                    }
                };
                actionCell.appendChild(returnBtn);
            } else {
                const returnedLabel = document.createElement('span');
                returnedLabel.textContent = 'คืนแล้ว';
                actionCell.appendChild(returnedLabel);
            }
        });
    } catch (err) {
        console.error('renderManageBorrowTable error', err);
    }
}

// ...existing code...
async function renderReturnedBorrowsForDeletion() {
    const tbody = document.getElementById('editReturnedBorrowsTbody');
    const selectAll = document.getElementById('selectAllReturnedCheckbox');
    if (!tbody) return;

    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;">กำลังโหลด...</td></tr>';
    try {
        // Try ordered query first; if Firestore doesn't allow orderBy(returnedAt) (missing field/index/etc)
        // fall back to simple query and sort in-memory.
        let docs;
        try {
            const snap = await borrowsCollection.where('returned', '==', true).orderBy('returnedAt', 'desc').get();
            docs = snap.docs;
        } catch (qerr) {
            console.warn('orderBy returnedAt failed, falling back to unordered fetch:', qerr);
            const snap = await borrowsCollection.where('returned', '==', true).get();
            docs = snap.docs.slice(); // copy
            docs.sort((a, b) => {
                const ad = a.data().returnedAt && typeof a.data().returnedAt.toDate === 'function'
                    ? a.data().returnedAt.toDate().getTime()
                    : 0;
                const bd = b.data().returnedAt && typeof b.data().returnedAt.toDate === 'function'
                    ? b.data().returnedAt.toDate().getTime()
                    : 0;
                return bd - ad;
            });
        }
        ...
        if (!docs || docs.length === 0) {
            ...
            const c = r.insertCell(0);
            c.colSpan = 6;
            c.textContent = 'ยังไม่มีรายการที่คืนแล้ว';
            c.style.textAlign = 'center';
            c.style.fontStyle = 'italic';
        ...
        for (const doc of docs) {
            ...
            chk.type = 'checkbox';
            chk.className = 'returned-delete-checkbox';
            chk.dataset.borrowId = doc.id;
            cbCell.appendChild(chk);

            const borrowedAt = (d.borrowedAt && typeof d.borrowedAt.toDate === 'function')
                ? d.borrowedAt.toDate().toLocaleString('th-TH')
                : (d.borrow_date || '-');

            const due = (d.dueDate && typeof d.dueDate.toDate === 'function')
                ? d.dueDate.toDate().toLocaleDateString('th-TH')
                : (d.dueDateString || '-');

            const returnedAt = (d.returnedAt && typeof d.returnedAt.toDate === 'function')
                ? d.returnedAt.toDate().toLocaleString('th-TH')
                : '-';
            ...
            const amountText = (typeof amountVal === 'number' && !isNaN(amountVal)) ? amountVal.toLocaleString('th-TH') : '-';
        ...
        // setup selectAll handler and keep it in sync
        if (selectAll) {
            selectAll.checked = false;
            selectAll.onchange = () => {
                const checks = tbody.querySelectorAll('input.returned-delete-checkbox');
                checks.forEach(c => { c.checked = selectAll.checked; });
            };
        }

        // update selectAll if individual checkboxes change
        tbody.querySelectorAll('input.returned-delete-checkbox').forEach(cb => {
            cb.onchange = () => {
                if (!selectAll) return;
                const all = Array.from(tbody.querySelectorAll('input.returned-delete-checkbox'));
                selectAll.checked = all.length > 0 && all.every(c => c.checked);
            };
        });

    } catch (e) {
        console.error('renderReturnedBorrowsForDeletion error', e);
        tbody.innerHTML = '';
        const r = tbody.insertRow();
        const c = r.insertCell(0);
        c.colSpan = 6;
        c.textContent = 'เกิดข้อผิดพลาดในการโหลดรายการ';
        c.style.color = 'red';
        c.style.textAlign = 'center';
    }
}
...
async function deleteSelectedReturnedBorrows() {
    const tbody = document.getElementById('editReturnedBorrowsTbody');
    if (!tbody) return;
    const checks = Array.from(tbody.querySelectorAll('input[type="checkbox"]:checked'));
    if (checks.length === 0) {
        alert('กรุณาเลือกอย่างน้อย 1 รายการที่จะลบ');
        return;
    }
    if (!confirm(`ยืนยันการลบ ${checks.length} รายการที่เลือก? การลบไม่สามารถย้อนกลับได้`)) return;
    try {
        let batch = db.batch();
        let counter = 0;
        for (const cb of checks) {
            const id = cb.dataset.borrowId;
            if (!id) continue;
            batch.delete(borrowsCollection.doc(id));
            counter++;
            if (counter >= 500) {
                await batch.commit();
                batch = db.batch();
        ...
        if (counter > 0) await batch.commit();
        // refresh views
        await renderReturnedBorrowsForDeletion();
        if (typeof renderManageBorrowTable === 'function') await renderManageBorrowTable();
        if (typeof renderBorrowTable === 'function') await renderBorrowTable();
    } catch (e) {
        console.error('deleteSelectedReturnedBorrows error', e);
        alert('เกิดข้อผิดพลาดขณะลบ รายละเอียดดูใน console');
    }
}
...
async function openStudentDetailModal(studentId) {
    ...
    // ยกเลิกการฟังข้อมูลเดิมก่อน (ป้องกันซ้ำ)
    if (window._studentDetailUnsub) window._studentDetailUnsub();

    // ดึงข้อมูลนักเรียนจาก Firestore แบบเรียลไทม์
    window._studentDetailUnsub = studentsCollection.doc(studentId).onSnapshot(async (doc) => {
        if (!doc.exists) {
        ...
        // ✅ อัปเดตข้อมูลนักเรียนใน modal (ตรวจสอบค่าก่อนทุกครั้ง)
        ...
        modalStudentId.textContent = student.id || doc.id || currentStudentId || '-';
        ...
        // ✅ ตรวจสอบว่ามี element รูปหรือไม่ ก่อนทำงานกับรูป
        const studentPhoto = document.getElementById("studentPhoto");
        if (!studentPhoto) {
            console.warn('#studentPhoto element ไม่พบ — ข้ามการโหลดรูป');
        } else {
            const imageFormats = ["jpg", "jpeg", "png", "webp"];
            let foundImage = false;
            const studentIdForImage = (student.id && String(student.id).trim()) ? student.id : (doc.id || currentStudentId);

            if (studentIdForImage) {
                for (let ext of imageFormats) {
                    const imagePath = `Students/${studentIdForImage}.${ext}`;
                    try {
                        // ✅ ป้องกัน error เมื่อไฟล์ไม่มีอยู่จริง
                        const res = await fetch(imagePath, { method: "HEAD" });
                        if (res && res.ok) {
                            studentPhoto.src = imagePath;
                            foundImage = true;
                            break;
                        }
                    } catch (err) {
                        // ✅ ไม่ต้องแสดง error ใน console เมื่อไฟล์ไม่พบ
                        // console.warn(`ไม่พบรูป: ${imagePath}`);
                    }
                }
            }

            // ✅ ถ้าไม่พบรูปเลย ใช้รูปเริ่มต้นแทน โดยไม่ทำให้ fetch error ขึ้น console
            if (!foundImage) {
                studentPhoto.src = "Image/default-profile.png";
            }

            // ✅ ตั้ง handler สำหรับ error ของรูป (เช่น ลิงก์รูปเสีย)
            studentPhoto.onerror = () => {
                if (!studentPhoto.src.includes('default-profile.png')) {
                    studentPhoto.src = "Image/default-profile.png";
                }
            };
        }
...
function openManageBorrowModal() {
    if (manageBorrowModal) manageBorrowModal.style.display = 'flex';
    renderManageBorrowTable();
}

function closeManageBorrowModal() {
    if (manageBorrowModal) manageBorrowModal.style.display = 'none';
}

function openAddBorrowModal() {
    if (!addBorrowModal) return;
    // populate student select from studentsData already loaded
    borrowerStudentSelect.innerHTML = '';

    // Add placeholder "-" as the default selected option (do not auto-select first student)
    const placeholderOpt = document.createElement('option');
    placeholderOpt.value = '';
    placeholderOpt.textContent = '-';
    placeholderOpt.selected = true;
    borrowerStudentSelect.appendChild(placeholderOpt);

    const sorted = (studentsData || []).slice().sort((a,b)=> (a.studentNumber||0)-(b.studentNumber||0));
    sorted.forEach(s => {
        const opt = document.createElement('option');
        opt.value = s.id;
        opt.textContent = `${s.studentNumber || '-'} - ${s.prefix || ''} ${s.name || ''} ${s.lastName || ''}`;
        borrowerStudentSelect.appendChild(opt);
    });

    // default datetime now
    const now = new Date();
    const localISO = new Date(now.getTime() - now.getTimezoneOffset()*60000).toISOString().slice(0,16);
    borrowDatetimeInput.value = localISO;
    daysUntilReturnInput.value = 7;
    computeDueDateFromDays();
    borrowAmountInput.value = 0;
    addBorrowModal.style.display = 'flex';
}

function closeAddBorrowModal() {
    if (addBorrowModal) addBorrowModal.style.display = 'none';
}

function computeDueDateFromDays() {
    const days = parseInt(daysUntilReturnInput.value,10);
    const dt = borrowDatetimeInput.value ? new Date(borrowDatetimeInput.value) : new Date();
    if (!isNaN(days)) {
        const due = new Date(dt.getTime());
        due.setDate(due.getDate() + days);
        // set dueDateInput value in yyyy-mm-dd
...
async function saveNewBorrower() {
    try {
        // Ensure borrower select exists and a borrower is chosen
        if (!borrowerStudentSelect) borrowerStudentSelect = document.getElementById('borrowerStudentSelect');
        const borrowerId = borrowerStudentSelect ? borrowerStudentSelect.value : '';
        if (!borrowerId) {
            alert('กรุณาเลือกผู้ยืมก่อน');
            if (borrowerStudentSelect) borrowerStudentSelect.focus();
            return;
        }

        const student = (studentsData || []).find(s => s.id === borrowerId) || {};
        const borrowerName = (student && (student.prefix || student.name || student.lastName))
            ? `${student.prefix || ''} ${student.name || ''} ${student.lastName || ''}`.trim()
            : '-';

        const borrowedAt = borrowDatetimeInput && borrowDatetimeInput.value ? new Date(borrowDatetimeInput.value) : new Date();
        const dueDate = dueDateInput && dueDateInput.value ? new Date(dueDateInput.value + 'T00:00:00') : null;
        const amount = parseFloat(borrowAmountInput ? borrowAmountInput.value : NaN);

        // Validate amount: must be a number > 0
        if (isNaN(amount) || amount <= 0) {
            alert('จำนวนเงินการยืมต้องมากกว่า 0 บาท');
            if (borrowAmountInput) borrowAmountInput.focus();
            return;
        }

        if (saveBorrowBtn) saveBorrowBtn.disabled = true;

        await borrowsCollection.add({
            borrowerId: borrowerId || null,
            borrowerName,
            amountBorrowed: amount,
            amountDeducted: 0,
            borrowedAt: firebase.firestore.Timestamp.fromDate(borrowedAt),
        ...
        console.error('saveNewBorrower error', err);
        alert('เกิดข้อผิดพลาดในการบันทึก');
    } finally {
        if (saveBorrowBtn) saveBorrowBtn.disabled = false;
    }
}
// ...existing code...
...
function toggleEditMode(isEditMode) {
    ...
    editStudentInfoBtn.addEventListener('click', async () => {
        if (!currentStudentId) {
            console.error('ยังไม่ได้เลือกนักเรียน');
            return;
        }

        try {
            const studentDoc = await studentsCollection.doc(currentStudentId).get();
            if (!studentDoc.exists) {
                console.error('ไม่พบนักเรียนที่เลือก');
                return;
            }

            const student = studentDoc.data();

            // ✅ ใส่ค่าลงใน input ที่อยู่ใน studentDetailsEdit
            ...
            // ✅ ซ่อน view-mode แล้วแสดง edit-mode
            document.getElementById('studentDetailsView').style.display = 'none';
            document.getElementById('studentDetailsEdit').style.display = 'block';

            // ✅ แสดงปุ่มบันทึก/ยกเลิก แทนปุ่มแก้ไข
            document.getElementById('editStudentInfoBtn').style.display = 'none';
            document.getElementById('saveStudentInfoBtn').style.display = 'inline-block';
            document.getElementById('cancelStudentInfoBtn').style.display = 'inline-block';
...
async function renderStudentFeeSummary() {
    ...
    // --- NEW: calculate borrowed amounts summary ---
    let totalBorrowed = 0;
    try {
        const borrowSnap = await borrowsCollection.get();
        borrowSnap.forEach(bdoc => {
            const bd = bdoc.data() || {};
            const amt = typeof bd.amountBorrowed === 'number' ? bd.amountBorrowed : (typeof bd.amount === 'number' ? bd.amount : 0);
            totalBorrowed += amt;
        });
    } catch (e) {
        console.error('Error loading borrows for summary:', e);
    }
    const totalOutstanding = grandTotalForAllStudents - totalBorrowed;

    // Update the title to include the grand total and borrowed summary
    const format = (n) => (typeof n === 'number' ? n.toLocaleString('th-TH') : '-');
    feeSummaryTitle.textContent = `สรุปค่าห้องของนักเรียน ห้อง 3/1 (ในแต่ละเดือน) - รวมทั้งหมด: ${format(grandTotalForAllStudents)} บาท — ถูกยืม: ${format(totalBorrowed)} บาท (คงเหลือ: ${format(totalOutstanding)} บาท)`;
}
...
async function switchContentSection(sectionName, month) {
    ...
    switch(sectionName) {
        ...
        case 'Studentborrow': btnId = 'showStudentborrowBtn'; break;
...
async function handleContactFormSubmit(event) {
    ...
    const serviceID = 'service_zgorhim';
    const templateID = 'template_m32e6xo';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showBorrowBtn = document.getElementById('showStudentborrowBtn');
    const borrowSection = document.getElementById('borrowTableContainer');

    const editManageBtn = document.getElementById('editManageBorrowModalBtn');
    const editModal = document.getElementById('editReturnedBorrowsModal');
    const closeBtn = document.getElementById('closeEditReturnedBorrowsModalBtn');

    const deleteBtn = document.getElementById('deleteSelectedReturnedBtn');

    if (editManageBtn && editModal) {
        editManageBtn.addEventListener('click', (e) => {
            e.preventDefault();
            editModal.style.display = 'flex';
            renderReturnedBorrowsForDeletion();
        });
    }
    if (closeBtn) closeBtn.addEventListener('click', () => { editModal.style.display = 'none'; });
    if (deleteBtn) deleteBtn.addEventListener('click', deleteSelectedReturnedBorrows);

    // Assign history modal specific controls
    historyVisitLimitInput = document.getElementById('historyVisitLimitInput');
    saveHistoryVisitLimitBtn = document.getElementById('SaveHistoryVisitLimitBtn');

    // If button exists, bind click to save function
    if (saveHistoryVisitLimitBtn) {
        saveHistoryVisitLimitBtn.addEventListener('click', async (e) => {
            e.preventDefault();
            try {
                const limit = historyVisitLimitInput ? parseInt(historyVisitLimitInput.value, 10) : NaN;
                if (isNaN(limit) || limit <= 0) {
                    alert('กรุณากรอกขีดจำกัดเป็นตัวเลขจำนวนเต็มมากกว่า 0');
                    return;
                }

                // Save under settings document using Firestore v8 API
                await appSettingsDocRef.set({
                    visitLimit: limit
                }, { merge: true });

                console.log('Visit limit saved:', limit);
                // Update other controls that may reflect this value
                if (historyLimitInput) historyLimitInput.value = limit;
                // Reload settings into UI (if implemented)
                if (typeof loadHistorySettings === 'function') await loadHistorySettings();

                // If auto-delete visits enabled, trigger trim (if function exists)
                if (typeof trimVisits === 'function') {
                    try { await trimVisits(limit); } catch (trimErr) { console.warn('trimVisits failed:', trimErr); }
                }

                // Provide minimal user feedback
                saveHistoryVisitLimitBtn.textContent = 'บันทึกแล้ว';
                setTimeout(() => { if (saveHistoryVisitLimitBtn) saveHistoryVisitLimitBtn.textContent = 'บันทึก'; }, 1200);

            } catch (error) {
                console.error('Error saving visit limit:', error);
                alert('เกิดข้อผิดพลาดในการบันทึกขีดจำกัด');
            }
        });
    }

    // Helper: SHA-256 hash string -> hex
async function hashStringSHA256(str) {
    ...
    try {
        ...
        console.warn('hashStringSHA256 failed', e);
        return null;
    }
}

// Check whether stored admin hash matches current ADMIN_PASSWORD
async function isAdminStoredValid() {
    try {
        const stored = localStorage.getItem('savedAdminPasswordHash');
        if (!stored) return false;
        if (!ADMIN_PASSWORD) {
            // try reload admin password from Firebase if not loaded
            if (typeof loadAdminPasswordFromFirebase === 'function') {
                await loadAdminPasswordFromFirebase();
            }
            if (!ADMIN_PASSWORD) return false;
        }
        const adminHash = await hashStringSHA256(String(ADMIN_PASSWORD));
        return adminHash && stored === adminHash;
    } catch (e) {
        console.error('isAdminStoredValid error', e);
        return false;
    }
}

// Save given password (hashed) to localStorage
async function saveAdminPasswordLocally(password) {
    const h = await hashStringSHA256(String(password));
    if (h) localStorage.setItem('savedAdminPasswordHash', h);
}

// Remove stored admin password
function clearLocalAdminPassword() {
    localStorage.removeItem('savedAdminPasswordHash');
}

// --- Replace existing handleSettingsAdminPasswordConfirm to save locally on success ---
async function handleSettingsAdminPasswordConfirm() {
    const password = settingsAdminPasswordInput ? settingsAdminPasswordInput.value : '';
    if (!password) {
        if (settingsAdminPasswordErrorMessage) settingsAdminPasswordErrorMessage.textContent = "กรุณากรอกรหัสผ่าน";
        return;
    }

    // Ensure we have current ADMIN_PASSWORD (load from firebase if needed)
    if (!ADMIN_PASSWORD && typeof loadAdminPasswordFromFirebase === 'function') {
        await loadAdminPasswordFromFirebase();
    }

    if (password !== ADMIN_PASSWORD) {
        if (settingsAdminPasswordErrorMessage) {
            settingsAdminPasswordErrorMessage.textContent = "รหัสผ่านแอดมินไม่ถูกต้อง";
        }
        return;
    }

    // Save hashed password locally for next visits
    try {
        await saveAdminPasswordLocally(password);
    } catch (e) {
        console.warn('Could not save admin password locally', e);
    }
    ...
    if (editFormSubmissionStudentList) editFormSubmissionStudentList.addEventListener('click', async (e) => {
        if (e.target.classList.contains('edit-student-answer-btn')) {
            ...
            try {
                ...
                try { recordWebsiteVisit(); } catch (e) { /* ignore */ }
    ...
    manageBorrowsBtn = document.getElementById('manageBorrowsBtn');
    borrowTableBody = document.getElementById('borrowTableBody');

    manageBorrowModal = document.getElementById('manageBorrowModal');
    closeManageBorrowModalBtn = document.getElementById('closeManageBorrowModalBtn');
    manageBorrowTableBody = document.getElementById('manageBorrowTableBody');
    openAddBorrowBtn = document.getElementById('openAddBorrowBtn');

    addBorrowModal = document.getElementById('addBorrowModal');
    closeAddBorrowModalBtn = document.getElementById('closeAddBorrowModalBtn');
    borrowerStudentSelect = document.getElementById('borrowerStudentSelect');
    borrowDatetimeInput = document.getElementById('borrowDatetimeInput');
    daysUntilReturnInput = document.getElementById('daysUntilReturnInput');
    dueDateInput = document.getElementById('dueDateInput');
    borrowAmountInput = document.getElementById('borrowAmountInput');
    saveBorrowBtn = document.getElementById('saveBorrowBtn');
    cancelAddBorrowBtn = document.getElementById('cancelAddBorrowBtn');
    ...
    if (manageBorrowsBtn) manageBorrowsBtn.addEventListener('click', openManageBorrowModal);
    if (closeManageBorrowModalBtn) closeManageBorrowModalBtn.addEventListener('click', closeManageBorrowModal);
    if (openAddBorrowBtn) openAddBorrowBtn.addEventListener('click', openAddBorrowModal);

    if (closeAddBorrowModalBtn) closeAddBorrowModalBtn.addEventListener('click', closeAddBorrowModal);
    if (cancelAddBorrowBtn) cancelAddBorrowBtn.addEventListener('click', closeAddBorrowModal);
    if (daysUntilReturnInput) daysUntilReturnInput.addEventListener('input', computeDueDateFromDays);
    if (borrowDatetimeInput) borrowDatetimeInput.addEventListener('change', computeDueDateFromDays);
    if (saveBorrowBtn) saveBorrowBtn.addEventListener('click', async (e) => { e.preventDefault(); await saveNewBorrower(); });
    ...
    if (navSettingBtn) {
    navSettingBtn.addEventListener('click', async (e) => {
        e.preventDefault();
        try {
            // โหลดรหัสแอดมินล่าสุดจาก Firestore ก่อนเสมอ
            if (typeof loadAdminPasswordFromFirebase === 'function') {
                await loadAdminPasswordFromFirebase();
            }
        } catch (loadErr) {
            console.warn('Could not reload admin password from Firebase:', loadErr);
            // ยังลองตรวจ/local fallback ต่อไป
        }

        try {
            if (await isAdminStoredValid()) {
                switchPage('settings');
            } else {
                openSettingsAdminPasswordModal();
            }
        } catch (err) {
            console.error('navSettingBtn handler error', err);
            openSettingsAdminPasswordModal();
        }
    });
}
    
    // Bind confirm button in password modal (ensure handler exists)
    ...
    // Logout admin button (clears local saved hash) - element added in index.html
    const logoutAdminBtn = document.getElementById('logoutAdminBtn');
    if (logoutAdminBtn) {
        logoutAdminBtn.addEventListener('click', () => {
            clearLocalAdminPassword();
            alert('ออกจากโหมดแอดมินเรียบร้อยแล้ว');
            // if currently in settings, redirect to home or class
...
loadAdminPasswordFromFirebase();
    ...
    if(navClassBtn) navClassBtn.addEventListener('click', (e) => { e.preventDefault(); switchPage('class'); switchContentSection('studentList') });
    ...
    if(showStudentborrowBtn) showStudentborrowBtn.addEventListener('click', () => switchContentSection('Studentborrow'));
    ...
    if (showBorrowBtn) {
    showBorrowBtn.addEventListener('click', async (e) => {
        e.preventDefault();
        try {
            // Load any required data first
            if (typeof loadStudentsFromFirebase === 'function') await loadStudentsFromFirebase();
            if (typeof loadPaymentRequirements === 'function') await loadPaymentRequirements();

            // Render summary table
            if (typeof renderBorrowTable === 'function') await renderBorrowTable();

            // Navigate / show UI: prefer class page + borrow section, fallback to manage modal
            try {
                switchPage('class'); // ensure class page visible (keeps navigation consistent)
            } catch (ignore) { /* ignore if switchPage not available */ }

            // Activate borrow section if present
            const borrowSectionElem = document.getElementById('borrowTableContainer');
            if (borrowSectionElem) {
                // deactivate other content sections inside class page then activate borrow section
                document.querySelectorAll('#classPageContainer .content-section').forEach(s => s.classList.remove('active'));
                borrowSectionElem.classList.add('active');
            } else {
                // fallback: open manage modal if section container missing
                if (typeof openManageBorrowModal === 'function') openManageBorrowModal();
            }

            // Ensure manage modal list is up-to-date if open
            if (manageBorrowModal && manageBorrowModal.style.display === 'flex') {
                if (typeof renderManageBorrowTable === 'function') await renderManageBorrowTable();
            }
            ...
            console.error('Error preparing borrow UI:', err);
            // fallback: try opening manage modal
            if (typeof openManageBorrowModal === 'function') openManageBorrowModal();
        }
    });
} else {
    console.warn('showStudentborrowBtn not found in DOM');
}
...
function setupQrTool() {
  ...
  downloadBtn.addEventListener('click', function() {
    ...
    // Events
historyWebsideBtn.addEventListener('click', (e) => {
    e.preventDefault();
...
clearHistorywebsideBtn.addEventListener('click', clearAllHistory);
refreshHistoryBtn.addEventListener('click', renderHistoryTable);

// เรียกใช้ฟังก์ชัน recordWebsiteVisit เมื่อหน้าเว็บโหลด
window.addEventListener('load', recordWebsiteVisit);
});
